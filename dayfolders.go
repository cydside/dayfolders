...
func createFolders() error {
	...
	for dateCursor.Before(dateEnd) || dateCursor.Equal(dateEnd) {
		...
		if dowPtr {
			shortDayName = fmt.Sprintf("%s", DayNames["it_IT"][short][dateCursor.Weekday()])
			shortMonthName = fmt.Sprintf("%s", MonthNames["it_IT"][long][dateCursor.Month()])
		}

		if (!onefPtr && !subfPtr) || subfPtr {
			s := []string{dayOfYear, dateCursor.Format(dayFormat), shortDayName}
			m := []string{dateCursor.Format(monthFormat), shortMonthName}
			err := os.MkdirAll(
				filepath.Join(
					pathPtr,
			...
			s := []string{dateCursor.Format("2006-01-02"), dayOfYear, shortDayName}
